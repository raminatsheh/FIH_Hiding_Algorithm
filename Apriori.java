...
public class Apriori extends Observable {
	...
	ArrayList<Itemset> all_frequent_itemsets = new ArrayList<Itemset>();

    public static void main(String[] args) throws Exception {
        Apriori ap = new Apriori(args, false);
    }
    ...
    private boolean first_scan = false;
    
    /** by default, Apriori is used with the command line interface */
    private boolean usedAsLibrary = false;
    /** used as itemset provider-- this is to help generate all itemsets and pass as itemsets*/
    private boolean itemset_provider = false;
    ...
    /** This is the main interface to provide itemsets*/
    
    public ArrayList<Itemset> Apriori_Itemset(String[] args){
    	itemset_provider = true;
    	return all_frequent_itemsets;
    }
    public  Apriori(String[] args, boolean all_itemsets) throws Exception
    {
        configure(args);
        go();
     ...
     * @param b 
     */
    public  Apriori(String[] args, boolean b, boolean first_scan) throws Exception
    {
        configure(args);
   ...
   /** outputs the current configuration
    ...
    private void calculateFrequentItemsets() throws Exception
    {
        ...
        int number_of_items_in_itemset = itemsets.get(0).length;
		...
		if (number_of_items_in_itemset == 1 && first_scan){
			for (int i = 0; i < itemsets.size(); i++) { 
			
				Itemset temp_itemset = convert_to_Itemset(itemsets.get(i));
				temp_itemset.support_level = count[i];
				if (temp_itemset.items.get(0) != 0){
		...
		for (int i = 0; i < itemsets.size(); i++) {
			...
			if ((count[i] / (double) (numTransactions)) >= minSup) {
				...
				if (number_of_items_in_itemset > 1){
					Itemset temp_itemset = convert_to_Itemset(itemsets.get(i));
					temp_itemset.support_level = count[i];
					all_frequent_itemsets.add(temp_itemset);
				}
    ...
    private Itemset convert_to_Itemset(int[] passed_itemset_int){
    	Itemset temp_itemset;
    	ArrayList<Integer> temp_items = new ArrayList<Integer>();
    	for (Integer item : passed_itemset_int){
    		temp_items.add(item);
    	}
    	temp_itemset = new Itemset(0, temp_items, "all");
    	return temp_itemset;
